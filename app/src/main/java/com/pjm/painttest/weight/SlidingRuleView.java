package com.pjm.painttest.weight;
...
public class SlidingRuleView extends View {
    ...
    private String currentNum = "123456789101112131415";
    /**
     * 用于滑动实例
     */
     ...
     * 指示器的高度
     */
    private final float greenPointHeight;
    /**
     * 起始值
     */
    private final int lineStartNum;
    /**
     * 绿色指针颜色
     */
    ...
    public SlidingRuleView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        //顶部的直线距离View顶部距离
        topDegreeLine = typedArray.getDimension(R.styleable.SlidingRuleView_topDegreeLine, dp2px(getContext(),65));
        //刻度间隔
        lineDegreeSpace = typedArray.getDimension(R.styleable.SlidingRuleView_lineDegreeSpace, dp2px(getContext(),10));
        //刻度起始值
        lineStartNum = typedArray.getInt(R.styleable.SlidingRuleView_startNum, 0);
        //刻度大数目 默认30
        lineCount = typedArray.getInt(R.styleable.SlidingRuleView_lineCount, 30);
        //默认选中位置
        defaultNumber = typedArray.getInt(R.styleable.SlidingRuleView_defaultNumber, lineStartNum);
        //计量单位
        unit = typedArray.getString(R.styleable.SlidingRuleView_unit);
        //长的刻度线条长度
        longDegreeLine = typedArray.getDimension(R.styleable.SlidingRuleView_longDegreeLine, dp2px(getContext(),35));
        //左间隔
        ruleLeftSpacing = typedArray.getDimension(R.styleable.SlidingRuleView_ruleLeftSpacing, dp2px(getContext(),5));
        //右间隔
        ruleRightSpacing = typedArray.getDimension(R.styleable.SlidingRuleView_ruleRightSpacing, dp2px(getContext(),5));
        //数字颜色
        ...
        //短刻度值的长度
        shortDegreeLine = typedArray.getDimension(R.styleable.SlidingRuleView_shortDegreeLine, dp2px(getContext(),20));
        //绿色指针粗细
        greenPointWidth = typedArray.getDimension(R.styleable.SlidingRuleView_greenPointWidth, dp2px(getContext(),4));
        //绿色指针粗细
        greenPointHeight = typedArray.getDimension(R.styleable.SlidingRuleView_greenPointHeight, dp2px(getContext(),20));
        //绿色指针高度
    ...
    private void initForegroundPaint(int w) {
        ...
        mForegroundPaint.setShader(new LinearGradient(0, 0, w / 2f, 0, 0X00FFFFFF, 0XFFFFFFFF, Shader.TileMode.MIRROR));
    ...
    private void init(Context context){
        ...
        trianglePaint.setColor(0xff2c2d30);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for(int i = 0;i <= lineCount * 10;i++){
            ...
            if(i % 10 == 0){
                ...
                //画刻度值
                String number = String.valueOf(lineStartNum + i / 10);
                //得到文字宽度
                ...
                //画短刻度
                paint.setStrokeWidth(3);
                canvas.drawLine(x, y + longDegreeLine - shortDegreeLine, x, y + longDegreeLine, paint);
            }
            x += lineDegreeSpace;
        }
        ...
        float startX = greenPointX + getScrollX();
        float startY = y + longDegreeLine - greenPointHeight;
        canvas.drawLine(startX, startY, startX, y + longDegreeLine, paint);

        //画倒三角形
        float triangleWidth = dp2px(getContext(), 10);
        float triangleHeight = dp2px(getContext(), 8);
        float triangleDistance = startY - dp2px(getContext(), 10);
        path.reset();
        path.moveTo(startX - triangleWidth/2f, triangleDistance);
        path.lineTo(startX + triangleWidth/2f, triangleDistance);
        path.lineTo(startX, triangleDistance + triangleHeight);
        path.close();
        ...
        //确定数字的值。用移动多少来确定
        currentNum = df.format(lineStartNum + (startX - leftBorder) / (lineDegreeSpace * 10.0f));
        //测量数字宽度
        float textWidth = paint.measureText(currentNum);
        canvas.drawText(currentNum, greenPointX - textWidth / 2 + getScrollX(), topDegreeLine - dp2px(getContext(),15 + 16), paint);
        //画测量的单位，大小是刻度值的2分之一
        paint.setTextSize(currentNumberSize / 2);
        canvas.drawText(unit, greenPointX + textWidth / 2 + getScrollX() + dp2px(getContext(),10), topDegreeLine - dp2px(getContext(),16 +16), paint);
        //滚动到默认选中位置
        if(!isSetDefault){
            int calcScrollX = (int) (leftBorder + (defaultNumber - lineStartNum) * (lineDegreeSpace) * 10 - greenPointX);
    ...
    public void scrollTo( int x, int y) {
        //左边界检测
        if (x <  leftBorder - getWidth() / 2f) {
            x = (int)(- getWidth() / 2 +leftBorder);
        }
        //有边界检测
        if (x + getWidth() / 2f> rightBorder) {
            x = (int)(rightBorder - getWidth() /2f - ruleRightSpacing);
        }
        if (x != getScrollX()) {
